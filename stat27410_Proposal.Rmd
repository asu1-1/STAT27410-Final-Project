...
To determine weights of allocation, we will run regression models to predict the future prices of each stock, calculating the percentage changes of each and assigning portfolio weights through convex optimization. \textit{STILL SUBJECT TO SPECIFIC MODEL CHANGES!}. For instance, for a certain stock A, our model will include the previous prices of stock A into our calculation for future price of A, while also incorporating the sector-ETF and SPX as measures of how well the economy is doing. For each tech stock, our model would look like:

$$\text{stock_price_future} = \beta_1\text{stock_price_current} + \beta_2\text{stock_price_previous} + \beta_3 \text{QQQ_current} +\\ \beta_4 \text{SPY_current} + \text{covariance with other tech stocks}$$
and for each consumer stock, it would look like:

$$\text{stock_price_future} = \beta_1\text{stock_price_current} + \beta_2\text{stock_price_previous} + \beta_3 \text{XLY_current} +\\ \beta_4 \text{SPY_current} + \text{covariance with other consumer stocks}$$

For all stocks, we will then compute the percentage change as

$$\text{Stock_price_percentage_change} = \frac{(\text{stock_price_future} - \text{stock_price_current})}{\text{stock_price_current}}$$

As opposed to a pure Auto-Regressive (AR) model, we will consider exogenous variables that aim to capture aspects of the economy that cannot be fully captured with our limited selection of stocks. This would allow for specific analysis while also factoring in more complex market trends. 

With are computed stock price percentage changes, we will then use convex optimization to determine the portfolio weights, where volume traded will play a role in managing the risk assigned to each stock. 
